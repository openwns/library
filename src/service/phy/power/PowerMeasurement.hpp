...
namespace wns { namespace service { namespace phy { namespace power {
	...
	public:
		...
		virtual const wns::Power getInterferencePower() const = 0; // interference contains noise here
		...
		/** @brief get noise power (constant) for the received PDU */
		virtual const wns::Power getNoisePower() const = 0;
		...
		// These methods were nice to have to save code at other places,
		// but unfortunately there were some obstacles to get there.
		//virtual const double getDistance() const = 0; // meters between sender and receiver [m]
		//virtual const rise::Station* getSourceStation() const = 0; // transmitter station. No dependency on rise allowed!
	};
