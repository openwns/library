...
DLREG::put(double value)
{
    if (numTrials_ + 1 < maxNrv_)
    {
        ...
        {
            ////Speetcl::warning("Warning: Wrong x value in DLREG::put !");
            return;
        }
...
DLREG::getResultLine(const int index, ResultLine& line) const
{
    ...
    {
        ////Speetcl::warning("DLREG::getResult():", "index out of range.");
        return;
    }
...
/*! Run Time Control (RTC) function, checks wether large sample conditions are
  ...
  values to collect before next p_rtc-call is determined by pd_skipInterval.*/
DLRE::Phase
DLREG::rtc()
{
    if (numTrials_ < 1000)
    ...
    else
    {
        ...
        while (i < indexMax_ - 1) // was pd_indexMax - 3 before
        {
            ...
            else
            {
                ...
                {
                    ////Speetcl::warning("Relative error of last level can`t be calculated: \n", "DLREG::status set to finish !!\n");
                    phase_ = finish;
                    reason_ = last;
                    return phase_;
                }
