...
#include "MarkovContinuousTime.hpp"
#include <WNS/logger/Logger.hpp>
#include <float.h>  // DBL_MAX

#ifndef WNS_MARKOVCHAIN_MARKOVCONTINUOUSTIMETRAFFIC_HPP
#define WNS_MARKOVCHAIN_MARKOVCONTINUOUSTIMETRAFFIC_HPP

namespace wns { namespace markovchain {

	/**
	 * @brief this class specializes MarkovContinuousTime
	 * by fixing the template T=TrafficSpec
	...
	class MarkovContinuousTimeTraffic :
		public MarkovContinuousTime<TrafficSpec>
	{
	public:
		/**
		 * @brief Creates an empty MarkovContinuousTimeTraffic.
		 * numberOfChains is needed.
		 */
		MarkovContinuousTimeTraffic(int _numberOfChains) :
			MarkovContinuousTime<TrafficSpec>(_numberOfChains),
			rateScale(1.0),
			meanRate(0.0),
			minRate(DBL_MAX),
			maxRate(0.0)
		{
			MESSAGE_SINGLE(VERBOSE,logger, "MarkovContinuousTimeTraffic(C=" << _numberOfChains << ") called");
		}

		/**
		 * @brief MarkovContinuousTimeTraffic destructor
		 */
		~MarkovContinuousTimeTraffic()
		{
			MESSAGE_SINGLE(VERBOSE,logger, "~MarkovContinuousTimeTraffic() called");
		}
		...
		calculateStateProbabilities()
		{
			...
			MarkovContinuousTime<TrafficSpec>::calculateStateProbabilities();
...
#endif // WNS_MARKOVCHAIN_MARKOVCONTINUOUSTIMETRAFFIC_HPP
