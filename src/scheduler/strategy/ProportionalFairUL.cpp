...
#include <WNS/scheduler/strategy/ProportionalFairUL.hpp>
...
STATIC_FACTORY_REGISTER_WITH_CREATOR(ProportionalFairUL,
				     StrategyInterface,
				     "ProportionalFairUL",
				     wns::PyConfigViewCreator);


ProportionalFairUL::ProportionalFairUL(const wns::pyconfig::View& config)
...
simTimeType
ProportionalFairUL::scheduleOneBurst(simTimeType burstStart, simTimeType burstLength, Group group, int subBand,
...
ProportionalFairUL::doStartScheduling(int subBands, int maxSpatialLayers, simTimeType slotLength)
{
	...
	// clearMap(); //bursts.clear();

	// init bit counters for all users
	//for (UserSet::const_iterator iter = activeUsers.begin(); // some users missing!
