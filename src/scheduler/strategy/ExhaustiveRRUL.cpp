...
#include <WNS/scheduler/strategy/ExhaustiveRRUL.hpp>
...
STATIC_FACTORY_REGISTER_WITH_CREATOR(ExhaustiveRRUL,
				     StrategyInterface,
				     "ExhaustiveRRUL",
				     wns::PyConfigViewCreator);

bool
ExhaustiveRRUL::everyGroupMemberWasServed(Group group)
{
...
void
ExhaustiveRRUL::doStartScheduling(int fChannels, int maxSpatialLayers, simTimeType slotLength)
{
	...
	MESSAGE_BEGIN(NORMAL, logger, m,"ExhaustiveRRUL::startScheduling called - Rx");
	m << "\n\t Channels:   " << fChannels
	...
	do {
		...
		MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - retrieved grouping from grouper:\n" << grouping.getDebugOutput());
		for (unsigned int group = 0; group < grouping.groups.size(); ++group) {
			...
			if (!unservedUsersLastRound)
			{
				...
				{
					MESSAGE_SINGLE(NORMAL, logger,"Skipping a group because of fairness");
					continue;
				}
			...
			MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - now scheduling group number " << group);

			if (currentGroup.size() == 0) {
				    MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - Empty group -> skipping");
				continue;
			}

			MESSAGE_BEGIN(VERBOSE, logger, m, "ExhaustiveRRUL::startScheduling - nextFreeSlots: ");
	    ...
	    // some initialization
			...
			MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - chose fSlot=" << fSlot);
			...
			for (Group::iterator iter = currentGroup.begin();
				 ...
				 ++iter) {
				MESSAGE_SINGLE(NORMAL, logger,"added to usersInGroup: " << colleagues.registry->getNameForUser(iter->first));
			...
			for (unsigned int i = 0; i < usersInGroup.size(); ++i) {
				...
				MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - user "
			...
			for (unsigned int user = 0; user < currentGroup.size(); ++user) {
				MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - now scheduling \tuser number " << user);
				...
				if (getSchedulerState()->powerControlType==PowerControlULSlave)
				{
					...
					phyModePtr = getSchedulerState()->currentState->strategyInput->defaultPhyModePtr;
					txPowerPerStream = getTxPower();
				...
				else
				{
					...
					MESSAGE_SINGLE(NORMAL, logger, "UL PHY mode for: " << colleagues.registry->getNameForUser(usersInGroup[user])
								<< " is : " << *phyModePtr<<" with SINR: "<<sinr);
				}
				...
				do { // try to place as many pdus from the current user as
					...
					*/
					...
					for (unsigned int cid = 0; cid < connections.size(); ++cid){
						MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL::startScheduling - now scheduling connection number " << connections[cid]);
...
//sdr					if (HoLms[cid] < slotLength - usedInBeam[user]) { // user fits in
						if (HoLms[cid] <= slotLength - usedInBeam[user] + slotLengthRoundingTolerance) { // user fits in
							bool firstPDU = false;
							PowerCapabilities power = colleagues.registry->getPowerCapabilities(usersInGroup[user]);
							MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL: txPower = " << power.nominalPerSubband);
							...
							MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL: a)  startTime " << currentBurst->start << ", stopTime = " << currentBurst->end);
							MESSAGE_SINGLE(NORMAL, logger,"ExhaustiveRRUL: startTime " << usedInBeam[user] << ", stopTime = " << usedInBeam[user]+HoLms[cid]);
							...
							compoundReady(fSlot,
										  ...
										  currentBurst,
										  *phyMode,
										  power.nominalPerSubband, //txPowerPerStream,
