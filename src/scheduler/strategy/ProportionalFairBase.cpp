...
ProportionalFairBase::groupExhausted(Group group) const
{
         ...
         iter != group.end(); ++iter)
    {
        ...
        ConnectionVector allConns = colleagues.registry->getConnectionsForUser(user);
        // if the queue for at least one of the users has run out of
        // data consider this group exhausted
        // user consists of 1..many connections
        bool empty=true;
        for (ConnectionVector::iterator iter = allConns.begin();
             iter != allConns.end();
             ++iter)
            if (colleagues.queue->queueHasPDUs(*iter)) empty=false;
        if (empty) {
            exhausted = true;
            break;
        }
